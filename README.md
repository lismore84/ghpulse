...
`update_all_stats.py` 现在会自动更新所有统计表（包括新的基础统计数据），无需指定参数。
...
### 数据采集与统计
- **数据采集**: 使用 GitHub Events API，实时数据流处理，代码在 `streaming_ingest.py` 中
- **统计更新**: 批量统计计算，代码在 `update_all_stats.py` 中
  - 计算热门仓库、活跃开发者、事件统计等各项指标
  - 包含新的基础统计数据（用户最后活跃时间、仓库最后事件时间等）
  - 可作为定时任务运行，例如每天执行一次
